...
)
{
  ...
  ## Hospitalisation probabilities may now be time-varying, so the conversion to
  ## Pr(hospitalised | symptomatic) is done per offspring in Step 2 after the
  ## relevant symptom-onset clock time is known.
  ...
  ## remain separate, even if the same trajectory is supplied to both.
  ...
  for (i in seq_along(symptomatic_indices)) {
    ...
    if (offspring_dataframe$class[si] == "HCW") {
      p_hosp_t <- resolve_probability(prob_hospitalised_hcw, t_onset, "prob_hospitalised_hcw")
    } else {
      p_hosp_t <- resolve_probability(prob_hospitalised_genPop, t_onset, "prob_hospitalised_genPop")
    }

    prob_hosp[i] <- prob_hosp_given_symptoms(prob_hosp = p_hosp_t, prob_symptomatic = prob_symptomatic)
  }
