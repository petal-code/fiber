...
#' \eqn{1 - \mathrm{hospital\_quarantine\_efficacy}(t)}. If
#' \code{hospital_quarantine_efficacy} is supplied as a time-varying function, it is resolved
#' at each candidate hospital transmission time.
...
#'   admission/ETU/quarantine at reducing post-admission transmission. If a function, it is resolved
#'   at the absolute calendar time of each candidate hospital infection.
...
offspring_function_genPop <- function(
  ...
  hospital_quarantine_efficacy = NULL,      # scalar or function(t): efficacy of hospitalisation at reducing transmission
  ...
  #         If hospital_quarantine_efficacy is time-varying, resolve it at the
  #         absolute calendar time of each candidate hospital transmission.
