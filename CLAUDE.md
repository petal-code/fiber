...
`out$sim_info$params` bundles the transmission / natural-history / OBV parameters the run used (everything except population/seeding/final-size/seed), so post-hoc tools can replay the model without re-threading ~40 arguments. Purely additive — no RNG is drawn and existing outputs are byte-for-byte unchanged.

### Leaky-OBV sensitivity (`estimate_leaky_onward()`)

`estimate_leaky_onward()` (`R/estimate_leaky_onward.R`, exported) is a **post-hoc** sensitivity estimator answering "what if OBV only *reduced* infectiousness rather than fully sterilizing?" — the standard reviewer concern that the sterilizing gate is optimistic. It runs **outside** `branching_process_main()`, taking that run's `prevented_completed` frame plus `sim_info$params`, and never perturbs the original trajectory or its RNG stream. Validity rests on the current **no-depletion** regime (`susceptible_deplete` is an unimplemented placeholder; `hcw_available` is tracking-only), which makes each averted infection's subtree an independent branching process — so forward-simulating them post-hoc is unbiased. **If depletion is ever wired into transmission, this decoupling breaks and the estimator must be revisited.**

Reinterpretation: the existing `efficacy(dpc)` is read as P(the drug *engages*) — who lands in `prevented_completed` and, recursively, who is engaged among their descendants — and `residual_transmissibility` (r ∈ [0,1]) is the fraction of normal transmission an engaged individual still produces. The forward sim reuses the offspring + `complete_offspring_info()` machinery with a **leaky** rule: an engaged case is *kept* (not removed) and transmits at `mn_offspring_* × r` on **all** routes (community/hospital/funeral), applied at **every** generation the OBV gate engages (reusing `apply_obv_pep_gate()` unchanged in its sterilizing mode and reading both its realised set and its `obv_pep_prevented_info` — their union split by whether OBV engaged). `r = 0` reproduces the sterilizing model's transmission (engaged transmit nothing) but still counts the averted index cases and their deaths (added infections = number averted, added deaths = `prevented_deaths`, no onward chains); `r = 1` means OBV has no transmission effect (≈ the no-OBV world). Index cases and their (fixed) deaths are added back per the agreed accounting. The result reports per-replicate and summarised `onward_*` (descendants only) and `added_*` (index + descendants) counts; a `max_tree_size` cap warns rather than silently truncating a supercritical sweep point.
