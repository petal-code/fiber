...
Default efficacy curve `obv_pep_efficacy_from_dpc()` is NHP-derived (E0 = 0.82 at dpc=0, decays to 0 at dpc ≥ 10). It is **separable**: `eff(dpc) = E0 · shape(dpc)` where `shape(dpc)` depends only on `d50, k, dpc_zero, max_dpc` and `shape(0) = 1`, so `E0` is a pure vertical scale equal to the peak (dpc=0) efficacy. The `obv_pep_efficacy` argument selects the model: **NULL or a scalar** use the built-in curve (a scalar IS the curve's `E0`); a **function(dpc)** is used as-is (e.g. `function(dpc) rep(c, length(dpc))` for a flat, DPC-independent efficacy — the scalar form no longer means "flat constant"). The curve's shape parameters (`E0`, `d50`, `k`, `dpc_zero`, `max_dpc`) can be swept without writing a closure by passing `obv_pep_efficacy_args` (a named list of overrides) to `branching_process_main()`; it applies to the built-in curve only (NULL or scalar `obv_pep_efficacy`), errors if combined with a function `obv_pep_efficacy` / if it names `E0` while `obv_pep_efficacy` is a scalar / given an unknown name, and is validated up front (e.g. `E0 ∈ [0, 1]`) so a bad sweep point fails before the run.
